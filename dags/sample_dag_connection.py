...
def sample_data_extract_func(**kwargs):
    ...
    mssql = MsSqlHook(mssql_conn_id='app_env_instanceid_mssql', schema='mssqldb-sales-demo-001')
    df = mssql.get_pandas_df(sql='select getdate()')
    with pd.option_context('display.max_rows', None, 'display.max_columns', None):  # more options can be specified also
        print(f'DDDDDDDDDDDDDDD:{df}')
